...
public class tuike extends JPanel implements ActionListener{
	...
	JButton jb=new JButton("退课");
	String id1;
	JLabel a=new JLabel("退课页面");
public tuike(String id) {
	try{
	...
	BackgroundPaneltuike jp=new BackgroundPaneltuike((new ImageIcon("C:\\Users\\11862\\Pictures\\ZM\\退课.png")).getImage());
	jp.add(boxH);
...
public void actionPerformed(ActionEvent c) {
	...
	String sql="delete from sc where cno='"+cno.getText()+"' and sno='"+id1+"'";
	...
	//System.out.println(cno.getText());
	 ...
	 if(cno.getText().equals("")){
			JOptionPane.showMessageDialog(tuike.this, "课程号不能为空");
		}
		else {
			...
			try{
				...
				if(!(rs.next())){
					...
					if(!(cn.getText().equals(rs.getString("cn").trim()))) {
						...
						if(!rs1.next()) {
							JOptionPane.showMessageDialog(this,"没选该课程");
						}
					else{
...
class BackgroundPaneltuike extends JPanel  
{  
    Image im;  
    public BackgroundPaneltuike(Image im)  
    {  
        this.im=im;  
