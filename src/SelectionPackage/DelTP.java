...
public class DelTP extends JPanel implements ActionListener {
	...
	public DelTP() {
		...
		BackgroundPanelDelTP jp;//创建面板
		jp=new BackgroundPanelDelTP((new ImageIcon("C:\\Users\\11862\\Pictures\\ZM\\背景.png")).getImage());
		jp.add(boxH);
	...
	public void actionPerformed(ActionEvent c){ 
		...
		if(obj==jb) {
			...
			sql2=
					...
					"	  as " + 
					"	    begin" + 
					"		delete from CTimetable where sy in(select sy from deleted) and term in(select term from deleted) and sy1 in(select sy1 from deleted) and dno in(select dno from deleted)" + 
					"		                             and cno in(select cno from deleted ) " + 
					"		end" 
					; 
			sql3="drop trigger TP_CT_delete";
			sql="delete from TP where sy='"+Sy.getText()+"' and term='"+Term.getText()+"' and sy1='"+Sy1.getText()+"'and dno='"+Dno.getText()+"' and cno='"+Cno.getText()+"'";
			...
			try{
			    ...
			    if(Sy.getText().equals("")||Term.getText().equals("")||Sy1.getText().equals("")||Dno.getText().equals("")||Cno.getText().equals("")){ //涓撲笟浠ｇ爜涓嶅彲浠ヤ负绌哄��
					JOptionPane.showMessageDialog(this,"学年、学期、年级、专业、课程名不能为空");
				}
				else {
					if(!(Term.getText().equals("1")||Term.getText().equals("2")))
						...
						if(!rs.next()){
							...
							JOptionPane.showMessageDialog(this, "教学计划删除成功");
						}
...
class BackgroundPanelDelTP extends JPanel  
{  
    Image im;  
    public BackgroundPanelDelTP(Image im)  
    {  
        this.im=im;  
