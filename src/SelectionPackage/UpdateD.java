...
public class UpdateD extends JPanel implements ActionListener{
	...
	//组件设置
		public UpdateD(){
			...
			BackgroundPanelUpdateD pCenter;//创建面板
			pCenter=new BackgroundPanelUpdateD((new ImageIcon("C:\\Users\\11862\\Pictures\\ZM\\改专业.png")).getImage());
			pCenter.add(boxH);
		...
		public void actionPerformed(ActionEvent c){
			...
			if(obj==jb){
				if(Dno.getText().equals("")){ //专业代码不可以为空值
					JOptionPane.showMessageDialog(this,"专业代码为空");
				}
				else{
					if(Dn.getText().equals("")){ //专业名称不可以为空值
						...
						sql1="update D set dn='"+Dn.getText()+"' where dno='"+Dno.getText()+"'";
						try{
							CONN co=new CONN();
...
class BackgroundPanelUpdateD extends JPanel  
{  
    Image im;  
    public BackgroundPanelUpdateD(Image im)  
    {  
        this.im=im;  
